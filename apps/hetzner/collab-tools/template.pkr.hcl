...
build {
  ...
  provisioner "file" {
    destination = "/opt/"
    source      = "apps/hetzner/collab-tools/files/opt/"
  }

  provisioner "file" {
    destination = "/var/"
    source      = "apps/hetzner/collab-tools/files/var/"
  }

  provisioner "shell" {
    environment_vars = ["DEBIAN_FRONTEND=noninteractive", "LC_ALL=C", "LANG=en_US.UTF-8", "LC_CTYPE=en_US.UTF-8"]
    scripts          = ["apps/shared/scripts/apt-upgrade.sh", "apps/hetzner/collab-tools/scripts/install.sh", "apps/shared/scripts/cleanup.sh"]
  }
}
